...
def gen_fetch_data(fetch_baselines_npz_path):
    '''Transform fetch data as generated by baselines code into our data format.

     Data format:

     states are stored in 'obs'  i.e. fetch_data['obs'][0] is 1 traj (type
     list) fetch['obs'][0][0]['observation'] is an arry of shape (25,)

     actions are stored in 'acs' as fetch_data['acs'][0] represents an action
     trajectory and fetch_data['acs'][0][0] represents the action at a step
    '''
    ...
    obstacles = np.array([])
    set_diff = np.array([])
    return env_data_dict, expert_traj_dict, obstacles, set_diff
...
def main(args):
    ...
    elif args.data_type == 'fetch_pick_and_place':
        env_data_dict, expert_data_dict, obstacles, set_diff = gen_fetch_data(
                args.fetch_baselines_npz)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fetch_baselines_npz', type=str, default='',
                        help='Path to fetch pick and place data as generated '\
                             'in baselines.')
