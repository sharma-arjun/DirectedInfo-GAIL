...
class CausalGAILMLP(BaseGAIL):
    ...
    def train_gail(self, num_epochs, results_pkl_path, gen_batch_size=1,
        ...
        for ep_idx in range(num_epochs):
            ...
            gen_traj_curr_epoch = {'state': [], 'action': [], 'context': []}
            ...
            while num_steps < gen_batch_size:
                ...
                if not args.use_goal_in_policy:
                    c_gen, expert_goal = self.get_c_for_traj(state_expert,
                                                             action_expert,
                                                             c_expert)
                    print(np.unique(np.argmax(c_gen[0,:,:], axis=1)))
                ...
                for t in range(expert_episode_len):
                    ...
                    curr_state_arr = np.array(next_state_feat, dtype=np.float32)[None, :]
                ...
                gen_traj_curr_epoch['context'].append(gen_traj_dict['c'])
