...
def directed_gail_expert(expert_traj_path):
    assert os.path.exists(expert_traj_path), "Expert traj path does not exist"
    from load_expert_traj import ExpertHDF5
    expert = ExpertHDF5(expert_traj_path, 17)
    expert.push(one_hot_action=False)
    states_actions = [np.hstack([expert.memory_no_tuple[i][0], expert.memory_no_tuple[i][1]]) \
            for i in range(len(expert.memory_no_tuple))]
    return np.array(states_actions).reshape(-1, 17 + 6)

# expert_traj, running_state = pickle.load(open(args.expert_traj_path, "rb"))
expert_traj = directed_gail_expert(args.expert_traj_path)
running_state = None
