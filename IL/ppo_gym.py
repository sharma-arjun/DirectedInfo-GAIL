...
from utils.misc_utils import assets_dir
...
if args.model_path is None:
    if is_disc_action:
        ...
        #policy_net = Policy(state_size=state_dim, action_size=0, latent_size=0, 
        #                   output_size=env_dummy.action_space.shape[0], 
        #                   hidden_size=128, output_activation=None)
        policy_net = Policy(state_dim, env_dummy.action_space.shape[0], log_std=args.log_std)

    #value_net = Value(state_size=state_dim, hidden_size=128)
...
"""create agent"""
agent = Agent(env_factory, policy_net, running_state=running_state, render=args.render, num_threads=args.num_threads, use_gpu=args.cuda)
...
def update_params(batch, i_iter):
    ...
    #a_mean, a_log_std, a_std = policy_net.forward(Variable(states, volatile=True))
    #fixed_log_probs = normal_log_density(Variable(actions), a_mean, a_log_std, a_std).data

    """get advantage estimation from the trajectories"""
    advantages, returns = estimate_advantages(rewards, masks, values, args.gamma, args.tau, args.cuda)
    ...
    for _ in range(optim_epochs):
        ...
        perm = LongTensor(perm).cuda() if args.cuda else LongTensor(perm)
